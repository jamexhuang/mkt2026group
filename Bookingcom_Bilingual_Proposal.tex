...
% Force Traditional Chinese-capable fonts to prevent missing glyph boxes.
...
\begin{longtable}{p{0.21\textwidth} p{0.18\textwidth} p{0.41\textwidth} p{0.16\textwidth}}
\toprule
...
Question mark & Main independent variable & Number of question marks in the post after removing URLs & \texttt{question} \\
...
\begin{equation}
\begin{aligned}
\text{Engagement}_i = {}& \beta_0 + \beta_1\text{TextLength}_i + \beta_2\text{Question}_i + \beta_3\text{Valence}_i \\
& + \beta_4\text{Hashtag}_i + \beta_5\text{Picture}_i + \beta_6\text{URL}_i + \beta_7\text{AtMention}_i + \varepsilon_i
\end{aligned}
\end{equation}

Conceptually, this specification tests whether the proposed textual features explain variation in engagement above and beyond visual or structural elements in the post. Because the dependent variable is a non-negative count-based sum and the sample shows substantial right skew, the main analysis may use
\[
\log\bigl(1 + \text{Engagement}_i\bigr)
\]
as the modeled dependent variable to reduce the influence of extreme observations and improve interpretability. The conceptual definition of engagement, however, remains the raw sum of likes, comments, and shares.
...
\begin{enumerate}
    ...
    \item Kumar, N., Qiu, L. and Kumar, S. (2022). A hashtag is worth a thousand words: An empirical investigation of social media strategies in trademarking hashtags. \textit{Information Systems Research}, 33(4), 1403--1427.
...
本研究聚焦於 Booking.com 貼文的「文字設計」，而非整體品牌策略。核心概念是：貼文怎麼寫，會影響使用者如何反應。既有文獻顯示，內容結構、互動線索、情緒語調與可發現性訊號，都可能影響社群回應。據此，本研究檢驗文字長度、問號、情緒正向程度與 hashtag 是否與更高互動有關，並控制圖片、URL 與 at-mention 等非文字因素。
...
第四，hashtag 可能提升可發現性、組織主題意義，並將貼文連結至更廣泛對話。Kumar、Qiu 與 Kumar（2022）指出，hashtag 對提升社群受眾互動具有關鍵作用。對 Booking.com 而言，hashtag 有助於旅遊內容被搜尋與被看見，從而提高互動。
...
\begin{longtable}{p{0.20\textwidth} p{0.18\textwidth} p{0.43\textwidth} p{0.15\textwidth}}
\toprule
構念 & 角色 & 操作型定義 & 資料欄位 \\
\midrule
\endfirsthead
...
問號 & 主要自變數 & 貼文（移除 URL 後）問號數量 & \texttt{question} \\
...
上述變數設定兼具理論與實務可行性。\texttt{length} 以清理後詞數而非原始字元數衡量，較能反映讀者可接收的實質文字資訊量。問號以計數而非虛擬變數處理，可捕捉不同程度的互動語氣。\texttt{valence} 以連續分數呈現語言正負向程度，暫不拆分為正面與負面兩個獨立構面。hashtag 以計數表示主題標記與可發現性強度。控制變數用以捕捉文字以外的結構因素。emoji 則保留於延伸模型，作為額外解釋力檢驗。
...
此模型用以檢驗：文字特徵是否能在視覺與結構因素之外，額外解釋互動差異。由於依變數為非負計數加總且右偏明顯，實證估計可採
\[
\log\bigl(1 + \text{Engagement}_i\bigr)
\]
以降低極端值影響並提升解讀性；但互動概念定義仍維持 likes、comments、shares 的原始加總。
