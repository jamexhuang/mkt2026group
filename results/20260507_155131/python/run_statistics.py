...
Comprehensive Statistical Analysis Script  v5.0.0  (2026-05-07)

v5.0.0 changes vs v4.0.0:
  * Added Negative Binomial (NB2) regression as a robustness check on the
    raw integer engagement count. Reports α-hat with LR-vs-Poisson test,
    AIC/BIC, McFadden pseudo-R², full coefficient table with IRR=exp(β),
    and an explicit OLS-vs-NB hypothesis-decision concordance table.
    Standard NB is sufficient at 13.6 % zero rate (104/766); ZINB is
    flagged only as a future-direction note in the report Limitations.
  * New artifacts: python/nb_regression_results.txt,
    python/nb_parameter_estimates.csv, plots/nb_pearson_residuals.png.
...
# v5.0.0: pin to the new release tag; can be overridden via env RESULT_TAG
_ts = os.environ.get("RESULT_TAG", "20260507_release")
...
# ═══════════════════════════════════════════════════════════════════════════════
print("=" * 70)
print("Booking.com Engagement Study — Statistical Analysis v5.0.0")
print("=" * 70)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  7b. Negative Binomial Robustness Check (v5.0.0)
# ═══════════════════════════════════════════════════════════════════════════════
rpt.h2("Table 3b. Negative Binomial Robustness Check")
rpt.p("<b>Purpose:</b> Verify that the OLS log-linear hypothesis decisions are "
      "robust to a count-data specification. We re-estimate the model as "
      "Negative Binomial (NB2) on raw <code>engagement</code>, evaluate "
      "overdispersion via an LR test against Poisson, and compare hypothesis "
      "decisions across the two models.")

# Pre-fit HC1 robust OLS for the OLS-side of the concordance table.
# (§11 below also refits with HC1 — that block is preserved for back-compat.)
ols_hc1 = smf.ols(formula, data=df_ana).fit(cov_type="HC1")

# ── Fit Poisson (reference) ─────────────────────────────────────────────────
nb_X = sm.add_constant(df_ana[ALL_X_COLS].astype(float))
nb_y = df_ana["engagement"].astype(float)

res_poisson = Poisson(nb_y, nb_X).fit(disp=False, maxiter=200)
ll_poisson = res_poisson.llf

# ── Estimate α via two complementary methods ───────────────────────────────
# Method 1: Method-of-moments (MoM) on the marginal distribution.
#   V(Y) = μ + α·μ²  ⇒  α̂ = (s²(Y) − ȳ) / ȳ²
#   More robust to outliers than CT auxiliary regression.
y_mean = float(nb_y.mean())
y_var = float(nb_y.var(ddof=1))
alpha_mom = max((y_var - y_mean) / (y_mean ** 2), 1e-6)
# Method 2: Cameron–Trivedi (1990) auxiliary regression (kept for diagnostics)
mu_p = res_poisson.fittedvalues
_lhs = (nb_y - mu_p) ** 2 - mu_p
_rhs = mu_p ** 2
alpha_ct = max(float(np.sum(_lhs * _rhs) / np.sum(_rhs ** 2)), 1e-6)
print(f"[INFO] α (MoM)  = {alpha_mom:.4f}")
print(f"[INFO] α (CT-aux) = {alpha_ct:.4f}")

# ── Fit NB2 via GLM with α_MoM (two-step is the stable choice here) ────────
# Joint NB MLE on this data is pathological: V/M ≈ 850, the likelihood
# surface is nearly flat in α and the optimizer drifts to absurd values
# (~10⁶) without improving fit. The Cameron–Trivedi auxiliary regression
# similarly inflates α because of one extreme outlier (max=2474).
# The MoM estimator α̂ = (s²−ȳ)/ȳ² is well-conditioned, gives a reasonable
# overdispersion magnitude, and yields stable coefficients via GLM.
alpha_hat = alpha_mom
nb_family = sm.families.NegativeBinomial(alpha=alpha_hat)
res_nb = sm.GLM(nb_y, nb_X, family=nb_family).fit(maxiter=200)
ll_nb = res_nb.llf
nb_method_used = (f"NB2 GLM with α fixed at the MoM estimator "
                  f"α̂_MoM = (Var(Y)−E[Y])/E[Y]² = {alpha_mom:.4f}")
res_nb_mle = None  # not used; flag for downstream null-fit branch

print(f"[INFO] NB method: {nb_method_used}")
print(f"[INFO] α̂ used   = {alpha_hat:.6f}")
print(f"[INFO] ll_nb    = {ll_nb:.4f}")

# ── HC1 robust SE on the GLM fit ───────────────────────────────────────────
try:
    res_nb_robust = sm.GLM(nb_y, nb_X, family=nb_family).fit(
        maxiter=200, cov_type="HC1")
    nb_robust_ok = True
except Exception as _e:
    print(f"[WARN] NB HC1 robust fit failed: {_e}; using default SE")
    res_nb_robust = res_nb
    nb_robust_ok = False

# ── LR test: overdispersion (NB vs Poisson, boundary χ-bar²) ───────────────
lr_stat = 2.0 * (ll_nb - ll_poisson)
# α=0 is on the boundary, so the asymptotic null is 0.5·χ²(0) + 0.5·χ²(1).
# The p-value is half of the standard one-sided χ²(1) p-value.
lr_p = 0.5 * (1 - stats.chi2.cdf(lr_stat, df=1)) if lr_stat > 0 else 1.0

# ── Likelihood-based AIC / BIC (manual; statsmodels GLM uses deviance-BIC)
_k_full = nb_X.shape[1]  # 9 betas (intercept + 8 IVs)
nb_aic_llf = -2.0 * ll_nb + 2.0 * _k_full
nb_bic_llf = -2.0 * ll_nb + _k_full * np.log(N)
poisson_aic_llf = -2.0 * ll_poisson + 2.0 * _k_full
poisson_bic_llf = -2.0 * ll_poisson + _k_full * np.log(N)

# ── McFadden pseudo-R² (NB null vs full, same fitting path as full model) ─
nb_null_X = pd.DataFrame({"const": np.ones(len(nb_y))}, index=nb_y.index)
try:
    if res_nb_mle is not None and not (np.isnan(res_nb_mle.llf) or np.isinf(res_nb_mle.llf)):
        # Discrete-model MLE path — refit null via discrete NegativeBinomial.
        res_nb_null = NegativeBinomial(nb_y, nb_null_X).fit(
            start_params=np.array([np.log(y_mean), alpha_hat]),
            method="newton", maxiter=200, disp=False)
        ll_null = res_nb_null.llf
    else:
        # GLM path — refit null with the same fixed α.
        nb_family_null = sm.families.NegativeBinomial(alpha=alpha_hat)
        res_nb_null = sm.GLM(nb_y, nb_null_X, family=nb_family_null).fit(maxiter=200)
        ll_null = res_nb_null.llf
    mcfadden = 1.0 - ll_nb / ll_null if ll_null != 0 else float("nan")
except Exception as _e:
    print(f"[WARN] NB null fit failed: {_e}; pseudo-R² unavailable")
    mcfadden = float("nan")

# ── Build NB parameter estimates table ──────────────────────────────────────
rpt.h3("Negative Binomial Parameter Estimates "
       f"({'HC1 robust SE' if nb_robust_ok else 'default SE — robust HC1 unavailable'})")
nb_param_rows = []
display_params = ["const"] + ALL_X_COLS
for pname in display_params:
    if pname not in res_nb_robust.params.index:
        continue
    coeff = res_nb_robust.params[pname]
    se_val = res_nb_robust.bse[pname]
    z_val = res_nb_robust.tvalues[pname]
    p_val = res_nb_robust.pvalues[pname]
    irr = np.exp(coeff)
    irr_lo = np.exp(coeff - 1.96 * se_val)
    irr_hi = np.exp(coeff + 1.96 * se_val)
    p_str = "<.0001" if p_val < 0.0001 else f"{p_val:.4f}"
    label_key = pname if pname != "const" else "Intercept"
    label = LABEL.get(label_key, label_key)
    hyp = HYP_MAP.get(label_key, "")
    if label_key in CV_COLS:
        label += " (control)"
    if hyp:
        label = f"{hyp}: {label}"
    nb_param_rows.append({
        "Variable": label_key,
        "Label": label,
        "Estimate": f"{coeff:.5f}",
        "Std Error": f"{se_val:.5f}",
        ...
        "Pr > |z|": p_str,
        "IRR = exp(β)": f"{irr:.4f}",
        "IRR 95% CI Lo": f"{irr_lo:.4f}",
        "IRR 95% CI Hi": f"{irr_hi:.4f}",
    })

df_nb_params = pd.DataFrame(nb_param_rows)
rpt.table(df_nb_params, index=False)

# ── Model-fit & overdispersion summary ─────────────────────────────────────
rpt.h3("NB Model Fit & Overdispersion")
nb_fit_data = {
    "Statistic": [
        "N", "NB fitting method", "Log-likelihood (NB)", "Log-likelihood (Poisson)",
        "LR statistic (2·ΔLL)", "LR p-value (½·χ²(1) boundary)",
        "Dispersion α (used)", "AIC (NB)", "BIC (NB)",
        "AIC (Poisson)", "BIC (Poisson)",
        "McFadden pseudo-R²", "Robust SE (HC1) used?",
    ],
    "Value": [
        f"{N}",
        nb_method_used,
        ...
        f"{ll_poisson:.4f}",
        f"{lr_stat:.2f}",
        "<.0001" if lr_p < 0.0001 else f"{lr_p:.6f}",
        f"{alpha_hat:.4f}",
        f"{nb_aic_llf:.2f}",
        f"{nb_bic_llf:.2f}",
        f"{poisson_aic_llf:.2f}",
        f"{poisson_bic_llf:.2f}",
        f"{mcfadden:.4f}" if not np.isnan(mcfadden) else "n/a",
        "Yes" if nb_robust_ok else "No (default SE)",
    ],
}
rpt.table(pd.DataFrame(nb_fit_data), index=False)

# ── OLS vs NB hypothesis-decision concordance ─────────────────────────────
rpt.h3("OLS vs NB Hypothesis Decision Concordance (H1–H5)")
PRED_SIGN = {"length": "+", "question": "+", "valence": "+",
             "hashtag": "+", "emoji": "+"}
...
for iv in IV_COLS:
    hyp = HYP_MAP[iv]
    pred = PRED_SIGN[iv]
    ols_b = ols_hc1.params[iv]
    ols_p = ols_hc1.pvalues[iv]
    nb_b = res_nb_robust.params[iv]
    nb_p = res_nb_robust.pvalues[iv]
    ols_dir = "+" if ols_b > 0 else "−"
    nb_dir = "+" if nb_b > 0 else "−"
    ols_sig = ols_p < 0.05
    nb_sig = nb_p < 0.05
    if ols_dir == pred and ols_sig:
        ols_decision = "Supported"
    elif ols_sig:
        ols_decision = "Not supported (wrong sign)"
    else:
        ols_decision = "Not supported (n.s.)"
    if nb_dir == pred and nb_sig:
        nb_decision = "Supported"
    elif nb_sig:
        nb_decision = "Not supported (wrong sign)"
    else:
        nb_decision = "Not supported (n.s.)"
    is_match = (ols_decision.startswith("Supported") ==
                nb_decision.startswith("Supported")) and (ols_dir == nb_dir)
    match_str = "✓ match" if is_match else "✗ divergent"
    if is_match:
        matches += 1
    concord_rows.append({
        "Hypothesis": hyp,
        ...
        "Predicted": pred,
        "OLS β": f"{ols_b:.4f}",
        "OLS p": "<.0001" if ols_p < 0.0001 else f"{ols_p:.4f}",
        "OLS decision": ols_decision,
        "NB β": f"{nb_b:.4f}",
        "NB p": "<.0001" if nb_p < 0.0001 else f"{nb_p:.4f}",
        "NB decision": nb_decision,
        "Match": match_str,
    })
rpt.table(pd.DataFrame(concord_rows), index=False)
rpt.p(f"<b>Concordance:</b> {matches}/{len(IV_COLS)} hypothesis decisions match between "
      f"OLS (HC1) and NB ({'HC1' if nb_robust_ok else 'default SE'}).")

# ── Persist NB parameter estimates as CSV ─────────────────────────────────
nb_csv_path = os.path.join(OUT_DIR, "nb_parameter_estimates.csv")
df_nb_params.to_csv(nb_csv_path, index=False, encoding="utf-8-sig")
print(f"[Saved] nb_parameter_estimates.csv")

# ── NB diagnostic plot: Pearson residuals vs fitted ─────────────────────────
nb_fitted = res_nb.predict(nb_X)
# NB2 variance: μ + α·μ²
nb_pearson = (nb_y - nb_fitted) / np.sqrt(nb_fitted + alpha_hat * nb_fitted ** 2)
fig_nb, ax_nb = plt.subplots(figsize=(8, 5))
ax_nb.scatter(np.log1p(nb_fitted), nb_pearson, alpha=0.3, s=10, color="#4a86c8")
ax_nb.axhline(y=0, color="red", linewidth=1, linestyle="--")
ax_nb.axhline(y=2, color="orange", linewidth=0.8, linestyle=":")
ax_nb.axhline(y=-2, color="orange", linewidth=0.8, linestyle=":")
ax_nb.set_xlabel("log(1 + Fitted Mean μ̂)")
ax_nb.set_ylabel("Pearson Residual")
ax_nb.set_title("NB Pearson Residuals vs Fitted (μ̂)")
fig_nb.tight_layout()
nb_plot_path = os.path.join(PLOT_DIR, "nb_pearson_residuals.png")
fig_nb.savefig(nb_plot_path, dpi=150, bbox_inches="tight")
rpt.img(fig_nb, "NB Pearson Residuals vs Fitted Mean (orange dotted = ±2)")
print(f"[Saved] plots/nb_pearson_residuals.png")

# ── Persist NB regression text dump ────────────────────────────────────────
nb_lines = []
nb_lines.append("=" * 70)
nb_lines.append("NEGATIVE BINOMIAL ROBUSTNESS CHECK — Booking.com Engagement Study v5.0.0")
nb_lines.append("DV: engagement (raw count)  |  Model: NB2 with log link")
nb_lines.append("=" * 70)
nb_lines.append(f"\nN = {N}")
nb_lines.append(f"NB fitting method         = {nb_method_used}")
nb_lines.append(f"Log-likelihood (NB)       = {ll_nb:.4f}")
nb_lines.append(f"Log-likelihood (Poisson)  = {ll_poisson:.4f}")
nb_lines.append(f"LR statistic (2·ΔLL)      = {lr_stat:.4f}")
nb_lines.append("LR p-value (½·χ²(1))      = "
                + ("<.0001" if lr_p < 0.0001 else f"{lr_p:.6f}"))
nb_lines.append(f"Dispersion α (used)       = {alpha_hat:.6f}  (>0 ⇒ NB > Poisson)")
nb_lines.append(f"  α (Cameron–Trivedi aux) = {alpha_ct:.6f}  (diagnostic only)")
nb_lines.append(f"AIC  NB={nb_aic_llf:.2f}   Poisson={poisson_aic_llf:.2f}")
nb_lines.append(f"BIC  NB={nb_bic_llf:.2f}   Poisson={poisson_bic_llf:.2f}")
if not np.isnan(mcfadden):
    nb_lines.append(f"McFadden pseudo-R²        = {mcfadden:.4f}")
nb_lines.append(f"Robust SE (HC1) used?     = {'Yes' if nb_robust_ok else 'No (default SE)'}")

nb_lines.append(f"\n{'─'*70}")
nb_lines.append("NB Coefficient Table")
nb_lines.append(f"{'─'*70}")
nb_lines.append(
    f"{'Variable':<22} {'Hyp':<6} {'Coeff':>10} {'SE':>10} "
    f"{'z':>8} {'p':>10} {'IRR':>9} {'Sig':>5}")
nb_lines.append("─" * 70)
for pname in ["const"] + ALL_X_COLS:
    if pname not in res_nb_robust.params.index:
        continue
    label_key = pname if pname != "const" else "Intercept"
    hyp_tag = HYP_MAP.get(label_key, "")
    coeff = res_nb_robust.params[pname]
    se_val = res_nb_robust.bse[pname]
    z_val = res_nb_robust.tvalues[pname]
    p_val = res_nb_robust.pvalues[pname]
    ...
    nb_lines.append(
        ...
        f"{coeff:>10.4f} {se_val:>10.5f} "
        f"{z_val:>8.2f} {p_val:>10.4f} {irr:>9.4f} {sig:>5}")
nb_lines.append("─" * 70)

nb_lines.append(f"\n{'─'*70}")
nb_lines.append("OLS vs NB Hypothesis Decision Concordance")
nb_lines.append(f"{'─'*70}")
nb_lines.append(f"{'Hyp':<5} {'Variable':<22} {'Pred':<5} "
                f"{'OLS dir':<8} {'OLS p':<10} {'NB dir':<8} "
                f"{'NB p':<10} {'Match':<10}")
nb_lines.append("─" * 70)
for r in concord_rows:
    nb_lines.append(
        f"{r['Hypothesis']:<5} {r['Variable']:<22} {r['Predicted']:<5} "
        f"{('+' if float(r['OLS β']) > 0 else '−'):<8} "
        f"{r['OLS p']:<10} "
        f"{('+' if float(r['NB β']) > 0 else '−'):<8} "
        f"{r['NB p']:<10} "
        f"{r['Match']:<10}")
nb_lines.append("─" * 70)
nb_lines.append(f"\nConcordance: {matches}/{len(IV_COLS)} hypotheses match between "
                f"OLS (HC1) and NB.")
nb_lines.append("Note: NB uses log link, so IRR = exp(β). For continuous X, "
                "IRR is multiplicative effect on E[Y] per unit; for binary X, "
                "IRR is the ratio of E[Y|X=1] / E[Y|X=0].")

nb_text = "\n".join(nb_lines)
nb_reg_path = os.path.join(OUT_DIR, "nb_regression_results.txt")
with open(nb_reg_path, "w", encoding="utf-8") as f:
    f.write(nb_text)
print(f"[Saved] nb_regression_results.txt")
print(nb_text)
...
reg_lines = []
reg_lines.append("=" * 70)
reg_lines.append("REGRESSION RESULTS  —  Booking.com Engagement Study  v5.0.0")
...
print("  ─ python/nb_regression_results.txt    (NB robustness check, v5.0.0)")
print("  ─ python/nb_parameter_estimates.csv   (NB coefficients with IRR)")
print("  ─ python/plots/*.png                  (diagnostic plots, incl. NB)")
print("  ─ plots/*.png                         (root-level plot copies)")
print("  ─ data/research_data_v4.csv           (analysis-input snapshot)")
print("  SAS input saved to: analysis/sas/input/")
print("  ─ sas_ready.csv               (all columns, research vars first)")
print("=" * 70)
print("\nDone. Script: run_statistics.py  v5.0.0")
