...
\small v1.1.0 - Full Empirical Results Included}
\author{}
\date{\today}
...
Table \ref{tab:results} (see Appendix) presents the results of the robust OLS estimation.
...
$R^2$ & 0.492 \\
\bottomrule
\multicolumn{2}{l}{\small Notes: Robust standard errors in parentheses.}\\
\multicolumn{2}{l}{\small $^{***} p<.001$, $^{**} p<.01$, $^* p<.05$.}
\end{tabular}

\vspace{2em}

\caption{穩健 OLS 迴歸結果：預測 $\log(Engagement)$}
...
$R^2$ & 0.492 \\
\bottomrule
\multicolumn{2}{l}{\small 註：括號內為穩健標準誤。}\\
\multicolumn{2}{l}{\small $^{***} p<.001$、$^{**} p<.01$、$^* p<.05$。}
\end{tabular}
\end{table}
